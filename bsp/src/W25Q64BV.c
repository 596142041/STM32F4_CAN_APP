#include "W25Q64BV.h"
SPI_FLASH W25Q64;
static u8 buffer[4096];//每一个扇区的缓存
/**********************************************************************
*函数名:static void SPIFLAH_GPIO_Config(void)
*输入参数:无;
*返回值:无;
*功能:初始化FLASH所用的IO;
************************************************************************/
...
void SPI_Flash_WR(unsigned long int addr,unsigned long int leng,unsigned char *data)
{
	...
	*/
	...
	unsigned short int sector_cnt;//用于记录当前所在的扇区数
	...
	{
		Read_Buffer((start.sector_num*W25Q64.PageSize),W25Q64.PageSize,buffer);//读取当前的扇区
	...
	{
		sector_cnt = start.sector_num;
		while(sector_cnt <= end.sector_num)
		{
			if(sector_cnt == start.sector_num)
			{
				Read_Buffer((sector_cnt*W25Q64.PageSize),W25Q64.PageSize,buffer);//读取当前的扇区
				for(cnt = start.sector_offset; cnt < W25Q64.PageSize;cnt++)//改变当前的扇区
				{
					buffer[cnt] = *data;
					data++;
				}
					//准备擦除当前的扇区
				Sector_4kb_Erase((sector_cnt*W25Q64.PageSize));//擦除当前所在的扇区
				Write_Page((sector_cnt*W25Q64.PageSize),buffer,4096);
			}
			else if(sector_cnt == end.sector_num)
			{
				Read_Buffer((sector_cnt*W25Q64.PageSize),W25Q64.PageSize,buffer);//读取当前的扇区
				for(cnt = 0; cnt < end.sector_offset;cnt++)//改变当前的扇区
				{
					buffer[cnt] = *data;
					data++;
				}
			...
			{
				Read_Buffer((sector_cnt*W25Q64.PageSize),W25Q64.PageSize,buffer);//读取当前的扇区
				for(cnt = 0; cnt < W25Q64.PageSize;cnt++)//改变当前的扇区
				{
					buffer[cnt] = *data;
					data++;
				}
